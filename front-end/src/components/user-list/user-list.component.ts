...
@Component({
  ...
  template: `
          ...
          <div class="overflow-x-auto">
              <table class="w-full text-left border-collapse">
                  ...
                  <tr class="bg-gray-50/50 border-b border-gray-100">
                      ...
                      <th class="px-8 py-4 text-xs font-semibold text-gray-400 uppercase tracking-wider text-right">
                          Ações
                      </th>
                  </tr>
                  </thead>
                      ...
                      @if (isLoading()) {
                          @for (item of [1, 2, 3]; track item) {
                              <tr class="animate-pulse">
                                  ...
                                  <td class="px-8 py-4">
                                      <div class="h-8 w-8 bg-gray-200 rounded ml-auto"></div>
                                  </td>
                              </tr>
                          }
                      ...
                      {{ user.usuario_perfil }}
                                  ...
                                  <td class="px-6 py-4 text-center">
                                      @if (user.is_blacklisted === 1) {
                                          <ng-template #blockedTooltip let-u>
                                              <div class="flex flex-col items-center gap-1">
                                                  <span class="font-semibold">Bloqueado</span>
                                                  <span class="text-gray-300">
               Libera em: <app-countdown [deadline]="u.deadline"></app-countdown>
            </span>
                                              </div>
                                          </ng-template>

                                          <span
                                                  (click)="handleBlacklistAction(user)"

                                                  [appTooltip]="user.can_be_removed === 0 ? blockedTooltip : 'Clique para remover da Blacklist'"
                                                  [tooltipContext]="user.can_be_removed === 0 ? { deadline: user.blacklist_deadline } : null"

                                                  [class.opacity-50]="user.can_be_removed === 0"
                                                  [class.cursor-not-allowed]="user.can_be_removed === 0"
                                                  [class.cursor-pointer]="user.can_be_removed === 1"
                                                  class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-red-50 text-red-600 border border-red-100 transition-all hover:bg-red-100">
      <i class="fa-solid fa-ban text-sm mr-1"></i> Bloqueado
    </span>
                                      } @else {
                                          <span appTooltip="Usuário Ativo"
                      ...
                      <i class="fa-solid fa-check text-sm mr-1"></i> Ativo
                    </span>
                                      }
                                  </td>
          ...
          <div class="px-8 py-4 border-t border-gray-100 bg-gray-50/50 flex justify-between items-center text-sm">
              <span class="text-gray-500">Mostrando <span
                      class="font-medium text-gray-900">{{ filteredUsers().length }}</span> registros</span>
              <div class="flex gap-1">
                  <button class="px-3 py-1 rounded border border-gray-200 bg-white text-gray-500 hover:bg-gray-50 disabled:opacity-50"
                          disabled>Anterior
                  </button>
                  <button class="px-3 py-1 rounded border border-gray-200 bg-white text-gray-500 hover:bg-gray-50">
                      Próximo
                  </button>
              </div>
          </div>
      ...
      @if (showConfirmModal() && userPendingRemoval()) {
          <app-confirm-modal
